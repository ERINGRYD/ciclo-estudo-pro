...
import { Store, Zap, Palette, Check, Lock, User, Map, Settings, Plus, X } from "lucide-react";
      ...
      <div className="fixed bottom-24 right-4 z-50">
        ...
        >
          {menuItems.map((item, index) => (
            <button
              key={item.label}
              className="flex items-center gap-3 bg-card border border-border rounded-full pl-4 pr-5 py-2.5 shadow-lg hover:bg-muted transition-colors whitespace-nowrap"
              style={{
        ...
        <button
          ...
          className={`w-14 h-14 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg flex items-center justify-center transition-transform duration-300 hover:scale-105 ${
            menuOpen ? "rotate-45" : ""
          }`}
        >
